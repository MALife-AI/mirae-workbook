...
└── outputs/                    ← 결과물 저장`}</Ref>
  ...
  },
  {
    ...
    mission: {
      ...
      promptTemplate: `/plan \${task} 프로그램을 만들건데, 리서치부터 보고서 작성, 결과물 생성까지 단계별 계획을 세워줘. 우리 팀은 \${dept}야.`,
      hints: ["AI가 계획을 보여주면 검토 후 승인하세요", "수정 요청도 가능합니다"],
      checklist: ["터미널에서 Plan 실행 완료"],
      autoChecks: null,
    },
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 16, justifyContent: "center", height: "100%" }}>
        ...
        <Code code={`// .claude/settings.local.json
{
  ...
  },
  {
    ...
    mission: {
      ...
      promptTemplate: `settings.local.json에 Bash, Read, Write, Edit 도구를 자동 허용 설정해줘. \${dept}에서 \${task} 자동화에 필요한 권한이야.`,
      hints: ["allowedTools에 도구명을 추가합니다", "위험한 도구는 제외하는 게 좋습니다"],
      checklist: ["settings.local.json 생성됨"],
      autoChecks: [
...
→ 바로 생성 시작...`}</div>
  ...
  },
  {
    ...
    mission: {
      ...
      promptTemplate: `CLAUDE.md 만들어줘.\n우리 팀은 \${dept}이고, \${task}를 자동화하는 프로젝트야.\n한국어로 작성하고, 브랜드 색상 오렌지 #F58220 사용하고, 질문하지 말고 바로 실행해줘.`,
      hints: ["브랜드 색상, 톤앤매너도 추가해보세요", "파일명 규칙이나 폴더 구조도 넣어보세요"],
      checklist: ["CLAUDE.md 파일 생성됨", "한국어 포함", "브랜드 색상 포함"],
      autoChecks: [
        { type: "file-exists", path: "CLAUDE.md" },
        { type: "file-contains", path: "CLAUDE.md", keyword: "한국어" },
        { type: "file-contains", path: "CLAUDE.md", keyword: "#F58220" },
      ],
    },
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 20, justifyContent: "center", height: "100%" }}>
...
· 템플릿 스타일 그대로 재사용`}</div>
  ...
  },
  {
    ...
    mission: {
      ...
      promptTemplate: `\${task} 작성 스킬을 만들어줘.\n\${dept}에서 사용하는 \${task}의 구조, 필수 포함 항목, 작성 순서를 담은 SKILL.md를 만들어줘.`,
      hints: ["필수 섹션과 작성 순서를 구체적으로 넣어보세요", "예시 데이터도 포함하면 품질이 올라갑니다"],
      checklist: ["Skill 파일 생성됨"],
      autoChecks: [
...
→ 전부 자동, 5분 완료`}</div>
  ...
  },
  {
    ...
    mission: {
      ...
      promptTemplate: `"/report" 슬래시 커맨드를 만들어줘.\n.claude/commands/report.md 파일로 만들어야 해.\n\${dept}에서 \${task}를 자동 생성하는 워크플로우야.\n웹 리서치 → 보고서 → 결과물 저장 순서로.`,
      hints: ["커맨드명은 업무에 맞게 변경해보세요", "여러 스킬을 순서대로 연결할 수 있습니다"],
      checklist: ["Command 파일 생성됨"],
      autoChecks: [
        { type: "any-exists", paths: [".claude/commands", "commands"] },
      ],
    },
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 20, justifyContent: "center", height: "100%" }}>
...
→ 안전한 파일로 재저장 ✓`}</div>
  ...
  },
  {
    ...
    mission: {
      ...
      promptTemplate: `파일 저장할 때 개인정보 있으면 차단하는 훅 만들어줘.\n\${dept}에서 \${task} 작업 시 주민번호, 전화번호가 포함되면 저장을 막아줘.`,
      hints: ["정규식으로 개인정보 패턴을 감지합니다", "차단 시 경고 메시지를 보여줍니다"],
      checklist: ["Hook 설정됨"],
      autoChecks: [
        { type: "any-exists", paths: [".claude/settings.local.json", "hooks", ".claude/hooks"] },
      ],
    },
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 20, justifyContent: "center", height: "100%" }}>
...
HTML + Python으로 간단하게 만들어줘.`}</Ref>
  ...
  {
    section: "모듈 1",
    title: "여기까지 성장한 내 프로그램",
    render: (props) => {
      // GrowthChart는 App에서 주입 (renderProps)
      const GrowthChart = props._GrowthChart;
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: 20, justifyContent: "center", height: "100%" }}>
          <div style={{ fontSize: 32, fontWeight: 900, color: M.tx, textAlign: "center" }}>
            여기까지 <span style={{ color: M.or }}>성장한 내 프로그램</span>
          </div>
          <div style={{ fontSize: 17, color: M.tx2, textAlign: "center" }}>
            같은 "보고서 만들어줘"를 말했는데, 도구가 쌓일수록 결과가 달라졌습니다
          </div>
          {GrowthChart}
        </div>
      );
  ...
  },
  {
    ...
    mission: {
      ...
      promptTemplate: `지금까지 \${dept}에서 \${task} 프로젝트로 진행한 사항들을 기억해줘. 그 다음 /compact 실행해줘.`,
  ...
  },
  {
    ...
    title: "체험: 노트 테이킹 실습",
    mission: {
      id: "notetaking", stage: 8, label: "노트 테이킹",
      description: "AI의 외부 기억장치를 활용합니다",
      promptTemplate: `이 프로젝트는 \${dept}의 \${task} 자동화야. 이 정보를 기억해줘. 다음에 물어보면 알려줘야 해.`,
      hints: ["메모는 세션 간에도 유지됩니다", "핵심 결정사항을 메모해두면 좋습니다"],
      checklist: ["메모 저장 완료"],
      autoChecks: null,
    },
    render: () => (
  ...
  },
  {
    ...
    mission: {
      id: "mcp-before", stage: 9, label: "MCP 전",
      description: "외부 도구 없이 AI의 한계를 체험합니다",
      promptTemplate: `2025년 \${task} 관련 최신 시장 규모 알려줘`,
      hints: ["AI가 최신 데이터를 모른다는 것을 확인하세요", "이 한계를 MCP로 해결합니다"],
      checklist: ["AI에게 질문 완료"],
      autoChecks: null,
    },
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 16, justifyContent: "center", height: "100%" }}>
  ...
  },
  {
    ...
    title: "체험: MCP 설치하기",
    mission: {
      id: "mcp-install", stage: 10, label: "MCP 설치",
      description: "웹 검색 + 데스크톱 자동화 MCP를 모두 설치합니다",
      promptTemplate: `claude mcp add web-search -- npx -y @anthropic-ai/mcp-web-search`,
      hints: [
        "1. 위 명령으로 웹검색 MCP 설치",
        "2. claude mcp add desktop-automation -- npx -y mcp-desktop-automation 으로 데스크톱 MCP도 설치",
        "3. 설치 후 /exit → claude 재시작 필요",
      ],
      checklist: ["MCP 설치 완료"],
      autoChecks: null,
    },
  ...
  },
  {
    ...
    title: "체험: MCP 권한 설정",
    mission: {
      id: "mcp-permission", stage: 11, label: "MCP 권한 설정",
      description: "설치한 MCP의 권한을 미리 설정합니다",
      promptTemplate: `.claude/settings.local.json에 MCP 권한을 추가해줘. mcp__web-search, mcp__desktop-automation을 허용하고, Bash, Read, Write, Edit 도구도 자동 허용해줘.`,
      hints: ["이미 settings.local.json이 있으면 permissions.allow에 추가됩니다"],
      checklist: ["settings에 MCP 권한 포함"],
      autoChecks: [
        { type: "any-contains", paths: [".claude/settings.local.json", ".claude/settings.json"], keyword: "mcp" },
      ],
    },
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 16, justifyContent: "center", height: "100%" }}>
        <div style={{ fontSize: 32, fontWeight: 900, color: M.tx, textAlign: "center" }}>MCP 권한 <span style={{ color: M.or }}>한번에 설정</span></div>
        <div style={{ fontSize: 18, color: M.tx2, textAlign: "center" }}>설치한 MCP의 권한을 미리 설정하면, 매번 승인하지 않아도 됩니다</div>
        <Code code={`// .claude/settings.local.json
{
  "permissions": {
    "allow": [
...
- MCP 도구가 있으면 "할 수 없다"고 답하지 말고 반드시 해당 도구를 호출하여 실행`} name="CLAUDE.md에 추가" />
  ...
  },
  {
    ...
    mission: {
      id: "mcp-after", stage: 12, label: "MCP 후",
      description: "아까와 같은 질문 — 이번엔 웹 검색 결과가 포함됩니다",
      promptTemplate: `2025년 \${task} 관련 최신 시장 규모 검색해줘`,
      hints: [
        "아까 MCP 없이 했던 같은 질문입니다",
        "이번엔 웹 검색으로 최신 데이터와 출처가 포함됩니다",
        "결과의 차이를 비교해보세요",
      ],
      checklist: ["MCP 적용 후 비교 완료"],
      autoChecks: null,
    },
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 14, justifyContent: "center", height: "100%" }}>
  ...
  },
  {
    ...
    mission: {
      id: "mcp-desktop", stage: 13, label: "데스크톱 자동화",
      description: "AI가 브라우저를 직접 조작해서 데이터를 수집합니다",
      promptTemplate: `Safari나 Chrome 브라우저를 실행해서 열어줘. 그 다음 \${task} 관련 정보를 검색하고, 화면에 보이는 내용을 캡처해서 읽어서 정리해줘. 모든 과정을 마우스와 키보드로 직접 조작해.`,
      hints: [
        "AI가 브라우저를 직접 열고 마우스/키보드로 조작합니다",
        "화면을 캡처해서 보이는 데이터를 읽어옵니다",
        "수집한 데이터를 텍스트로 정리해줍니다",
        "사이트 접속이 안 되면 다른 금융 사이트로 변경해보세요",
      ],
      checklist: ["데스크톱 크롤링 체험 완료"],
      autoChecks: null,
    },
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 16, justifyContent: "center", height: "100%" }}>
...
3. outputs/ 폴더에 Word 보고서로 정리`}</Ref>
  ...
  // ─── 최종 실습: 나만의 프로그램 만들기 ───
  {
    ...
    render: ({ projectPath, setProjectPath, projectNotice, setProjectNotice }) => {
      const handleNewFolder = async () => {
        if (!isTauri()) return;
        const home = await tauriInvoke("run_shell", { command: "echo $HOME" });
        const newPath = home.trim() + "/Documents/my-program";
        try {
          await tauriInvoke("set_project_dir", { path: newPath });
          setProjectPath(newPath);
          setProjectNotice("새 프로젝트 폴더 생성됨: " + newPath);
        } catch (e) {
          setProjectNotice("폴더 생성 실패: " + e);
        }
      };
      return (
      <div style={{ display: "flex", flexDirection: "column", gap: 20, justifyContent: "center", height: "100%" }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ background: `linear-gradient(135deg, ${M.or}22, #fbbf2422)`, border: `1px solid ${M.or}44`, borderRadius: 8, padding: "6px 20px", fontSize: 14, fontWeight: 700, color: M.or, letterSpacing: 2, display: "inline-block", marginBottom: 16 }}>BOSS STAGE</div>
          ...
          <div style={{ fontSize: 18, color: M.tx2, maxWidth: 600, margin: "0 auto", lineHeight: 1.7 }}>
            모듈 1~2에서는 강사가 제시한 예제를 따라했습니다.<br/>
            이제부터는 <strong style={{ color: M.or }}>처음부터 끝까지 혼자</strong> 만듭니다.
          </div>
        </div>

        {/* 새 프로젝트 폴더 */}
        <div style={{ ...card({ borderLeft: `4px solid #86efac` }), padding: "16px 20px" }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: "#86efac", marginBottom: 10 }}>새 프로젝트 폴더에서 시작</div>
          <div style={{ fontSize: 15, color: M.tx2, marginBottom: 12 }}>모듈에서 만든 파일과 섞이지 않도록 깨끗한 폴더에서 시작합니다.</div>
          <button onClick={handleNewFolder}
            style={{ background: `linear-gradient(135deg, #059669, #10b981)`, color: "#fff", border: "none", borderRadius: 10, padding: "12px 24px", fontSize: 15, fontWeight: 800, cursor: "pointer", boxShadow: "0 4px 12px #05966944" }}>
            새 프로젝트 폴더 만들기
          </button>
          {projectNotice && (
            <div style={{ marginTop: 10, fontSize: 13, color: "#86efac", fontFamily: "monospace" }}>
              {projectNotice}
            </div>
          )}
        </div>
  ...
  },
  {
    ...
    mission: {
      id: "final-plan", stage: 14, label: "BOSS: Plan",
      description: "처음부터 끝까지 혼자 만듭니다 — 먼저 설계!",
      promptTemplate: ``,
      hints: [
        "/plan 으로 시작해서 만들고 싶은 프로그램을 자유롭게 설명하세요",
        "내 역할, 자동화할 업무, 원하는 결과물을 포함하면 좋습니다",
        "AI가 설계안을 보여주면 검토 후 승인하세요",
      ],
      checklist: ["Plan 설계 완료"],
      autoChecks: null,
    },
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 16, justifyContent: "center", height: "100%" }}>
  ...
  },
  {
    ...
    mission: {
      id: "final-claudemd", stage: 15, label: "BOSS: CLAUDE.md",
      description: "내 프로젝트의 규칙서를 만듭니다",
      promptTemplate: ``,
      hints: [
        "CLAUDE.md 만들어줘 로 시작하세요",
        "언어, 톤, 색상, 보안 규칙 등 프로젝트 규칙을 자유롭게 넣으세요",
        "한번 만들면 매번 자동 적용되므로 꼼꼼하게 작성할수록 좋습니다",
      ],
  ...
  },
  {
    ...
    mission: {
      id: "final-skill", stage: 16, label: "BOSS: Skill",
      description: "내 업무의 반복 절차를 매뉴얼로 만듭니다",
      promptTemplate: ``,
      hints: [
        "[업무명] 스킬을 만들어줘 로 시작하세요",
  ...
  },
  {
    ...
    mission: {
      id: "final-command", stage: 17, label: "BOSS: Command",
      description: "한 마디로 전체 워크플로우를 실행하는 명령어",
      promptTemplate: ``,
      hints: [
        "/[원하는 명령어] 커맨드를 만들어줘 로 시작하세요",
        "어떤 작업을 어떤 순서로 실행할지 설명하면 됩니다",
  ...
  },
  {
    ...
    mission: {
      id: "final-hook", stage: 18, label: "BOSS: Hook",
      description: "안전장치를 설치합니다",
      promptTemplate: ``,
      hints: [
        "파일 저장 시 검사할 규칙을 만들어달라고 하세요",
        "개인정보, 민감 데이터 등 차단하고 싶은 패턴을 설명하면 됩니다",
        "업무 특성에 맞는 검사 규칙을 자유롭게 추가하세요",
      ],
      checklist: ["Hook 설정됨"],
      autoChecks: [
        { type: "any-contains", paths: [".claude/settings.local.json", ".claude/settings.json"], keyword: "hooks" },
      ],
    },
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 16, justifyContent: "center", height: "100%" }}>
  ...
  },
  {
    ...
    mission: {
      id: "final-run", stage: 19, label: "BOSS: 실행!",
      description: "만든 Command를 실행하고 결과물을 확인합니다",
      promptTemplate: ``,
      hints: [
        "방금 만든 /명령어 를 입력하세요",
        "결과물이 만족스럽지 않으면 Skill을 수정하고 다시 실행해보세요",
        "반복할수록 품질이 올라갑니다",
      ],
      checklist: ["결과물 생성됨"],
      autoChecks: null,
...
export default function App() {
  ...
  const showDeptInput = !personalization.isOnboarded && slide.section === "모듈 1";
      ...
      <nav style={{ width: sidebarW, minWidth: 140, maxWidth: 500, background: M.bg3, borderRight: `1px solid ${M.bd}`, display: "flex", flexDirection: "column", padding: "16px 0", position: "relative", flexShrink: 0 }}>
                  ...
                  <div style={{ paddingLeft: 14, marginTop: 1 }}>
                    {sec.slides.map((sl) => (
                      <button key={sl.index} onClick={() => setPage(sl.index)}
                        ...
                        {SLIDES[sl.index].mission && missionProgress.isCompleted(SLIDES[sl.index].mission.id)
                          ? <span style={{ color: "#86efac" }}>✓ </span>
                          : SLIDES[sl.index].mission
        ...
        {personalization.isOnboarded && (
          <div
            ...
            title="클릭하여 부서/업무 재설정"
            style={{ padding: "4px 12px", textAlign: "center", cursor: "pointer" }}>
            <span style={{ fontSize: 12, color: M.or, background: M.or + "18", borderRadius: 6, padding: "2px 8px" }}>
              🏢 {personalization.deptName}
            </span>
          </div>
        )}
          ...
          <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
            ...
            <div style={{ width: "45%", minWidth: 280, borderRight: `1px solid ${M.bd}` }}>
              <Suspense fallback={<div style={{ color: M.tx2, padding: 20, fontFamily: "monospace", background: M.bg3, height: "100%" }}>터미널 로딩 중...</div>}>
                <NativeTerminal style={{ height: "100%", borderRadius: 0, border: "none" }} fontSize={termFontSize} darkMode={darkMode} onPtyReady={({ write }) => { termWriteRef.current = write; }} />
              </Suspense>
            </div>
            {/* 미리보기/코드/출력 (우) */}
