    ...
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 24, justifyContent: "center", height: "100%" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {conceptCard("1️⃣", "Node.js 설치", "nodejs.org 접속 → LTS 버전 다운로드 → 설치 완료", "#059669")}
          <div style={{ ...card() }}>
            <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <Cmd cmd="npm install -g @anthropic-ai/claude-code" desc="터미널에서 실행" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, justifyContent: "center", height: "100%" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div style={{ ...card({ borderLeft: `4px solid ${M.tx3}` }) }}>
            ...
            <div style={{ background: M.bg3, borderRadius: 8, padding: 14, fontSize: "var(--code-font-size, 18px)", fontFamily: "'JetBrains Mono',monospace", color: M.tx3, lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{`Sure! What kind of report
...
I'll be happy to help!`}</div>
          ...
          <div style={{ ...card({ borderLeft: `4px solid #86efac` }) }}>
            ...
            <div style={{ background: M.bg3, borderRadius: 8, padding: 14, fontSize: "var(--code-font-size, 18px)", fontFamily: "'JetBrains Mono',monospace", color: "#86efac", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{`최신 보험 시장 동향을 주제로
보고서를 작성하겠습니다.
    ...
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 20, justifyContent: "center", height: "100%" }}>
        ...
        <div style={{ ...card({ borderLeft: `4px solid ${M.or}` }), padding: "20px 24px" }}>
          ...
          <textarea
            defaultValue={`CLAUDE.md 만들어줘. 미래에셋생명 보고서 자동화 프로젝트야.\n한국어, 브랜드 색상 오렌지 #F58220, 질문하지 말고 바로 실행하게 해줘`}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, justifyContent: "center", height: "100%" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div style={{ ...card({ borderLeft: `4px solid ${M.tx3}` }) }}>
            ...
            <div style={{ background: M.bg3, borderRadius: 8, padding: 14, fontSize: "var(--code-font-size, 18px)", fontFamily: "'JetBrains Mono',monospace", color: M.tx3, lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{`한국어 ✓ 브랜드 색상 ✓
...
· 템플릿 무시하고 새 스타일`}</div>
          ...
          <div style={{ ...card({ borderLeft: `4px solid #86efac` }) }}>
            ...
            <div style={{ background: M.bg3, borderRadius: 8, padding: 14, fontSize: "var(--code-font-size, 18px)", fontFamily: "'JetBrains Mono',monospace", color: "#86efac", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{`한국어 ✓ 브랜드 색상 ✓
  ...
  },
  {
    ...
    render: ({ tmplStatus, setTmplStatus } = {}) => {
      ...
      const handleCopy = async () => {
        if (!tauri) return;
        setTmplStatus("working");
        try {
          const msg = await tauriInvoke("copy_templates_to_project", {});
          setTmplStatus("done:" + msg);
        } catch (e) {
          setTmplStatus("error:" + e);
        }
      };
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: 20, justifyContent: "center", height: "100%" }}>
          <div style={{ fontSize: 32, fontWeight: 900, color: M.tx, textAlign: "center" }}>템플릿 파일 <span style={{ color: M.or }}>준비</span></div>
          <div style={{ fontSize: 18, color: M.tx2, textAlign: "center" }}>Skill이 참조할 미래에셋생명 공식 양식을 작업 폴더에 복사합니다</div>
          <div style={{ ...card({ borderLeft: `4px solid ${M.or}` }), padding: "16px 20px" }}>
            <div style={{ fontSize: 16, color: M.tx3, marginBottom: 12 }}>복사되는 파일: 미래에셋생명_A/B/C.docx, 미래에셋생명_A/B/C.pptx (6개)</div>
            <button onClick={handleCopy} disabled={!tauri || tmplStatus === "working"}
              style={{ background: tmplStatus.startsWith("done") ? "#059669" : M.or, color: "#fff", border: "none", borderRadius: 8, padding: "12px 24px", cursor: tauri ? "pointer" : "default", fontSize: 16, fontWeight: 700, width: "100%" }}>
              {tmplStatus.startsWith("done") ? "✓ " + tmplStatus.slice(5) : tmplStatus === "working" ? "복사 중..." : "📁 템플릿 파일 복사"}
            </button>
          </div>
          <div style={{ ...card(), padding: "12px 16px" }}>
    ...
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 20, justifyContent: "center", height: "100%" }}>
        ...
        <div style={{ ...card({ borderLeft: `4px solid #059669` }), padding: "20px 24px" }}>
          ...
          <textarea
            defaultValue={`경쟁사 분석 스킬 만들어줘. 삼성생명, 한화생명, 교보생명 비교하고, 뉴스/신상품/실적/전략 4가지로 수집해서 비교표로 정리하는 절차를 담아줘`}
            style={{ background: M.bg3, border: `1px solid ${M.bd}`, fontFamily: "'JetBrains Mono',monospace", color: M.or, borderRadius: 8, padding: "14px", width: "100%", resize: "vertical", fontSize: 14, lineHeight: 1.8, boxSizing: "border-box" }}
          />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, justifyContent: "center", height: "100%" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div style={{ ...card({ borderLeft: `4px solid ${M.tx3}` }) }}>
            ...
            <div style={{ background: M.bg3, borderRadius: 8, padding: 14, fontSize: "var(--code-font-size, 18px)", fontFamily: "'JetBrains Mono',monospace", color: M.tx3, lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{`1. "퇴직연금 보고서 만들어줘"
   → 결과 확인, 수정 요청
...
3. "규제 위반 없는지 검토해줘"
   → 총 3번 요청, 30분 소요`}</div>
          ...
          <div style={{ ...card({ borderLeft: `4px solid #86efac` }) }}>
            ...
            <div style={{ background: M.bg3, borderRadius: 8, padding: 14, fontSize: "var(--code-font-size, 18px)", fontFamily: "'JetBrains Mono',monospace", color: "#86efac", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{`/report 퇴직연금
→ 한 줄이면 끝!
    ...
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 20, justifyContent: "center", height: "100%" }}>
        ...
        <div style={{ ...card({ borderLeft: `4px solid ${M.or}` }), padding: "20px 24px" }}>
          <div style={{ fontSize: 15, fontWeight: 800, color: M.or, marginBottom: 12 }}>터미널에 입력할 프롬프트</div>
          <textarea
            defaultValue={`/report 커맨드 만들어줘.\n웹 리서치 → 보고서 → PPT 순서로 자동 생성하는거`}
            style={{ background: M.bg3, border: `1px solid ${M.bd}`, fontFamily: "'JetBrains Mono',monospace", color: M.or, borderRadius: 8, padding: "14px", width: "100%", resize: "vertical", fontSize: 14, lineHeight: 1.8, boxSizing: "border-box" }}
          />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          ...
          <div style={{ ...card(), padding: "12px 16px" }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: M.or, marginBottom: 6 }}>사용하는 방법</div>
            <div style={{ fontSize: 15, color: M.tx3, lineHeight: 1.8 }}>
              만들어진 Command는<br/>
              /report 퇴직연금 시장 동향<br/>처럼 직접 실행합니다.
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, justifyContent: "center", height: "100%" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div style={{ ...card({ borderLeft: `4px solid ${M.tx3}` }) }}>
            ...
            <div style={{ background: M.bg3, borderRadius: 8, padding: 14, fontSize: "var(--code-font-size, 18px)", fontFamily: "'JetBrains Mono',monospace", color: M.tx3, lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{`보고서가 저장되었습니다.
...
→ 사람이 검토하기 전까진 모름`}</div>
          ...
          <div style={{ ...card({ borderLeft: `4px solid #86efac` }) }}>
            ...
            <div style={{ background: M.bg3, borderRadius: 8, padding: 14, fontSize: "var(--code-font-size, 18px)", fontFamily: "'JetBrains Mono',monospace", color: "#86efac", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{`🚫 Hook이 파일 저장을 차단!
    ...
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 20, justifyContent: "center", height: "100%" }}>
        ...
        <div style={{ ...card({ borderLeft: `4px solid #fbbf24` }), padding: "20px 24px" }}>
          ...
          <textarea
            defaultValue={`파일 저장할 때 개인정보 있으면 차단하는 훅 만들어줘`}
            style={{ background: M.bg3, border: `1px solid ${M.bd}`, fontFamily: "'JetBrains Mono',monospace", color: M.or, borderRadius: 8, padding: "14px", width: "100%", resize: "vertical", fontSize: 14, lineHeight: 1.8, boxSizing: "border-box" }}
          />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
...

  {
    ...
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 20, justifyContent: "center", height: "100%" }}>
        ...
        <Ref title="실습 프롬프트">{`Python으로 Word 보고서 자동 생성 프로그램을 만들어줘.

기능:
- 주제를 입력하면 자동으로 보고서 생성
- 구성: 표지, 요약, 현황, 분석, 시사점, 제언
- 미래에셋 오렌지색 (#F58220) 헤더
- outputs/ 폴더에 날짜_주제.docx 형식으로 저장`}</Ref>
      </div>
    ),
  },
  {
    ...
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 24, justifyContent: "center", height: "100%" }}>
        ...
        <div style={{ ...card() }}>
          <div style={{ fontSize: 18, color: M.tx3, marginBottom: 12 }}>만든 프로그램을 이렇게 개선해보세요</div>
          {["표지 페이지를 추가해줘", "미래에셋 오렌지 색상 헤더를 넣어줘", "페이지 번호를 넣어줘", "목차를 자동 생성해줘", "표에 테두리와 색상을 추가해줘"].map((t, i) => (
...
- MCP 도구가 있으면 "할 수 없다"고 답하지 말고 반드시 해당 도구를 호출하여 실행`} name="CLAUDE.md에 추가" filePath="CLAUDE.md" />
  ...
  {
    section: "모듈 2",
    title: "Slash Command란?",
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 24, justifyContent: "center", height: "100%" }}>
        <div style={{ fontSize: 34, fontWeight: 900, color: M.tx, textAlign: "center" }}>Slash Command란?</div>
        {conceptCard("⚡", "앱 단축키처럼, 명령어 하나로 자동화", "/report 입력 하나로 리서치 → 보고서 → PPT까지 전부 자동 실행!", M.or)}
        <div style={{ ...card(), textAlign: "center" }}>
          <div style={{ fontSize: 22, color: M.tx3, marginBottom: 8 }}>기존 방식</div>
          <div style={{ fontSize: 18, color: M.tx2 }}>리서치 요청 → 보고서 요청 → PPT 요청 → ...</div>
          <div style={{ fontSize: 36, margin: "16px 0", color: M.or }}>→</div>
          <div style={{ fontSize: 22, color: "#86efac" }}>Slash Command 방식</div>
          <div style={{ fontSize: 24, color: M.or, fontWeight: 900, fontFamily: "monospace" }}>/report 퇴직연금 시장</div>
          <div style={{ fontSize: 18, color: "#86efac", marginTop: 8 }}>전부 자동 완료!</div>
        </div>
      </div>
    ),
  },
  {
    section: "모듈 2",
    title: "/report 명령어 만들기",
    render: () => (
      <div style={{ display: "flex", flexDirection: "column", gap: 20, justifyContent: "center", height: "100%" }}>
        <div style={{ fontSize: 32, fontWeight: 900, color: M.tx, textAlign: "center" }}>/report 명령어 <span style={{ color: M.or }}>만들기</span></div>
        <Code code={COMMAND_FILES[".claude/commands/report.md"]} name="commands/report.md" filePath=".claude/commands/report.md" />
        <SetupFiles files={COMMAND_FILES} title="명령어 생성" description="/report 자동화 명령어" />
        <Cmd cmd="/report 2025 퇴직연금 시장 동향" desc="실행 예시" />
      </div>
    ),
  },
  {
      ...
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: 12, justifyContent: "center", height: "100%" }}>
          ...
          <textarea
            ...
            rows={8}
            style={{ background: M.bg3, border: `1px solid ${M.bd}`, fontFamily: "'JetBrains Mono',monospace", color: M.or, borderRadius: 8, padding: 14, width: "100%", resize: "vertical", fontSize: "var(--code-font-size, 18px)", lineHeight: 1.7, outline: "none", boxSizing: "border-box" }}
          />
          <div style={{ fontSize: 15, color: M.tx3, textAlign: "center" }}>프롬프트를 자유롭게 수정한 뒤, 터미널에 붙여넣으세요. CLAUDE.md + Skill + Command + Hook이 한 번에 생성됩니다.</div>
        </div>
      );
    },
  },
  ...
  return (
    <div style={{ borderRadius: 12, overflow: "hidden", border: `1px solid ${editing ? M.or + "88" : M.bd}`, margin: "16px 0", transition: "border .2s" }}>
      ...
      {editing ? (
        ...
        <pre style={{ background: M.bg3, padding: 16, margin: 0, overflowX: "auto", fontFamily: "'JetBrains Mono', monospace", fontSize: "var(--code-font-size, 18px)", lineHeight: 1.7, color: M.tx }}>{displayContent}</pre>
      )}
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, margin: "6px 0" }}>
      <div style={{ flex: 1, display: "flex", alignItems: "center", background: M.bg3, borderRadius: 8, border: `1px solid ${M.bd}`, overflow: "hidden" }}>
        ...
        <code style={{ flex: 1, color: M.or, fontSize: "var(--code-font-size, 18px)", fontFamily: "'JetBrains Mono', monospace", padding: "8px 12px", whiteSpace: "nowrap", overflow: "auto" }}>{cmd}</code>
      </div>
...
function Ref({ title, children }) {
  ...
  return (
    <div style={{ borderRadius: 12, overflow: "hidden", border: `1px solid ${M.bd}`, margin: "16px 0" }}>
      ...
      <pre style={{ background: M.bg3, padding: 16, margin: 0, overflowX: "auto", fontFamily: "'JetBrains Mono', monospace", fontSize: "var(--code-font-size, 18px)", lineHeight: 1.8, color: M.tx2, whiteSpace: "pre-wrap" }}>{children}</pre>
    </div>
  );
}
...
1. Use the \`report-writer\` skill to create a docx report on "$ARGUMENTS"
2. Read the generated docx and create a matching pptx presentation using the same template style
3. Use the \`compliance-check\` skill to review the generated report for regulatory issues
4. Fix any compliance issues found and regenerate if needed
5. Output final file list and summary
6. After completion, suggest: "Want me to \\\`/competitor-watch\\\` to add competitive context?"`,
};
...
export default function App() {
  ...
  const sectionCheatsheets = {
    ...
    "도입: 시연": [
      ...
      { label: "생성된 파일 확인", cmd: "ls outputs/" },
      { label: "파일 열기 (macOS)", cmd: "open outputs/*.docx" },
    ],
    "모듈 1": [
      { label: "Claude Code 실행", cmd: "claude" },
      { label: "체험: CLAUDE.md 없이", cmd: "보고서 만들어줘" },
      { label: "CLAUDE.md 확인", cmd: "cat CLAUDE.md" },
      { label: "체험: Skill 없이", cmd: "퇴직연금 보고서 만들어줘" },
      { label: "Skill 파일 확인", cmd: "cat .claude/skills/report-writer/SKILL.md" },
      { label: "Skill 목록 확인", cmd: "ls .claude/skills/" },
  ...
  const slideCheatsheets = {
    ...
    "CLAUDE.md 만들기": [
      { label: "CLAUDE.md 확인", cmd: "cat CLAUDE.md" },
      { label: "CLAUDE.md 편집", cmd: "cat CLAUDE.md" },
    ],
    "체험: CLAUDE.md 적용 후": [
      { label: "같은 명령 다시 (즉시 적용)", cmd: "보고서 만들어줘" },
    ],
    "CLAUDE.md에 Skill 연결": [
      { label: "CLAUDE.md 현재 내용 확인", cmd: "cat CLAUDE.md" },
    ],
    "체험: Skill 없이": [
    ...
    "Skill 파일 만들기": [
      { label: "Skill 목록 확인", cmd: "ls .claude/skills/" },
      { label: "보고서 Skill 확인", cmd: "cat .claude/skills/report-writer/SKILL.md" },
      { label: "경쟁사 Skill 확인", cmd: "cat .claude/skills/competitor-watch/SKILL.md" },
      { label: "컴플라이언스 Skill 확인", cmd: "cat .claude/skills/compliance-check/SKILL.md" },
    ],
    "체험: Skill 적용 후": [
      { label: "같은 명령 다시 (즉시 적용)", cmd: "퇴직연금 보고서 만들어줘" },
    ],
    "실습: Hook 직접 만들기 Step 1": [
      { label: "Claude Code 실행", cmd: "claude" },
      { label: "Hook 만들기 요청", cmd: "개인정보 보호 Hook을 만들어줘. 파일을 저장할 때마다 주민번호, 전화번호가 있으면 차단해줘." },
    ],
    "실습: Hook 직접 만들기 Step 2": [
      { label: "Hook 테스트", cmd: "테스트용 보고서를 만들어봐. 내용에 이걸 포함해줘: \"고객 홍길동, 주민번호 901215-1234567, 연락처 010-1234-5678\"" },
      { label: "Hook 설정 확인", cmd: "cat .claude/settings.json" },
      { label: "이메일 감지 추가", cmd: "이메일 주소도 감지하면 경고만 표시하게 해줘" },
    ],
    "실습: 보고서 생성기": [
      { label: "Claude Code 실행", cmd: "claude" },
      { label: "보고서 생성기", cmd: "Python으로 Word 보고서 자동 생성 프로그램을 만들어줘. 주제를 입력하면 제목, 요약, 본문, 결론이 포함된 .docx 파일을 outputs/에 생성하는 프로그램이야" },
    ],
    "체험: MCP 없이": [
      { label: "Claude Code 실행", cmd: "claude" },
  ...
  // 슬라이드 제목으로 매칭 시도, 없으면 섹션 폴백
  const titleMatch = Object.keys(slideCheatsheets).find(k => slide.title.includes(k));
  const cheatItems = titleMatch ? slideCheatsheets[titleMatch] : (sectionCheatsheets[slide.section] || []);
    ...
    <div style={{ display: "flex", height: "100vh", background: M.bg, color: M.tx, fontFamily: "'Noto Sans KR',-apple-system,sans-serif", overflow: "hidden", "--code-font-size": codeFontSize + "px" }}>
      <style>{`
        ...
        *{box-sizing:border-box}
        [data-copyable]{font-size:var(--code-font-size, 18px) !important}
              ...
              <div key={sec.section} style={{ marginBottom: 2 }}>
                <button
                  ...
                  style={{
                    ...
                    fontSize: Math.max(10, codeFontSize - 2), transition: "all .15s", gap: 6,
                  }}>
                    ...
                    {sec.slides.map((sl) => (
                      <button key={sl.index} onClick={() => setPage(sl.index)}
                        style={{
                          ...
                          fontSize: Math.max(9, codeFontSize - 4), marginBottom: 1, transition: "all .1s",
          ...
          <>
            ...
            >
              <div ref={slideContentRef} style={{ width: slideScale < 1 ? `${100 / slideScale}%` : "100%", maxWidth: slideScale < 1 ? 900 / slideScale : 900, transform: `scale(${slideScale})`, transformOrigin: "top center", fontSize: "0.92em" }}>
                {slide.render({ skillTab, setSkillTab, deptTab, setDeptTab, projectPath, setProjectPath, projectNotice, setProjectNotice, tmplStatus, setTmplStatus })}
              </div>
            </div>
